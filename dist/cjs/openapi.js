...
var isValidSchema = (schema) => schema && typeof schema === "object" && (import_typebox.Kind in schema && schema[import_typebox.Kind] !== "Unknown" || schema.type || schema.properties || schema.items || schema.anyOf || schema.oneOf || schema.allOf || schema.$ref || schema.const !== void 0 || schema.enum);
...
var unwrapSchema = (schema, mapJsonSchema, io = "input") => {
  ...
  schema.type || // @ts-ignore
  schema.properties || // @ts-ignore
  schema.items || // @ts-ignore
  schema.anyOf || // @ts-ignore
  schema.oneOf || // @ts-ignore
  schema.allOf || // @ts-ignore
  schema.$ref || // @ts-ignore
  schema.const !== void 0 || // @ts-ignore
  schema.enum))
    return schema;
